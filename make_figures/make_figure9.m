%Make figure 9 in the PIG calving melt rate manuscript
%Single column showing the evolution of T-S contours along meridional cross section with calving for P=800m
%
...
ax_fontsize = 10;
figure(1); clf;
fig = gcf; fig.Position(3:4) = [400, 800];
...
%%%%%%%%%%%%%%%%%%%% Plots %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
positions = zeros(10,4);
positions(:,1) = 0.05;
positions(:,2) = flip(linspace(0.05, 0.85,10));
height = diff(positions(:,2));
height = height(1);
positions(:,4) = -height;
positions(:,3) = 0.9;
...
%plot in column
for p = 1:sz
ax(p) = subplot('Position', squeeze(positions(p,:))); hold on
ax(p).XTick = [];
ax(p).YTick = [];
box on
